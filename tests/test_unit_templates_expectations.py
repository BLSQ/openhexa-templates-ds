"""Unit tests for the TemplatesExpectations class.
...
from libs.templates__expectations import TemplatesExpectations
...
def test_init_with_invalid_dataset_type():
    ...
    with pytest.raises(ValueError, match="dataset should be a pandas dataframe"):
        TemplatesExpectations(dataset="not-a-df", expectations_yml_file=None)
...
def test_init_with_invalid_file_type(sample_df: pd.DataFrame):
    ...
    with pytest.raises(ValueError, match="expectations_yml_file should be a string"):
        TemplatesExpectations(dataset=sample_df, expectations_yml_file=123)


def test_missing_yaml_file(sample_df: pd.DataFrame):
    """Raise FileNotFoundError if expectations.yml file does not exist."""
    validator = TemplatesExpectations(sample_df, "non_existent.yml")
...
def test_invalid_yaml_syntax(sample_df: pd.DataFrame, tmp_path: str):
    ...
    validator = TemplatesExpectations(sample_df, str(bad_file))
...
def test_missing_dataframe_section(sample_df: pd.DataFrame, tmp_path: str):
    ...
    validator = TemplatesExpectations(sample_df, str(file))
