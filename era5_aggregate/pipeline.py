...
def era5_aggregate(
    ...
    """Aggregate ERA5 daily data to weekly, epidemiological weekly and monthly data."""
...
def get_daily(
    ...
    """Aggregate raw measurements to daily data for a given variable.

    Parameters
    ----------
    input_dir : Path
        Input directory containing raw grib files
    ...
    -------
    pl.DataFrame
        Polars DataFrame containing daily aggregated data
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for file in input_dir.glob("*.grib"):
            ...
            if zipfile.is_zipfile(file):
                with zipfile.ZipFile(file.as_posix(), "r") as zip_:
                    bytes_ = zip_.read("data.grib")
                with Path(tmpdir, file.name).open("wb") as f:
                    f.write(bytes_)
