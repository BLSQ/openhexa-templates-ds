...
## IMPORTANT NOTE.
This pipeline is not mantained by the DS team. We decided that it was too complex and not flexible enough. Our plan in the future is to divide this pipeline into two parts: one for extraction and another for loading, with a more generic transformation step in between that can be configured by the user. 
