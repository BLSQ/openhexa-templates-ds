...
class PyramidMatcher:
    ...
    def __init__(self, matcher: BaseMatcher | None = None, logger: logging.Logger | None = None):
        ...
        self.reference_pyramid: pl.DataFrame
        self.candidate_pyramid: pl.DataFrame
    ...
    def run_matching(
        ...
        for level in levels_to_match:
            ...
            data_matched, candidate_no_match_level, reference_no_match_level = self._match_level(
    ...
    def _define_schema_match(
        ...
        return (
            ...
            + [f"{self.prefix_reference_data}{attr}" for attr in attributes_level["reference"]]
            + ["score_" + target_level]
            + [f"{self.prefix_candidate_data}{attr}" for attr in attributes_level["candidate"]]
        )
    ...
    def _get_attributes(self, levels_to_match: list, matching_col_suffix: str) -> dict:
        ...
        for level in levels_to_match:
            ...
            if len(attributes_ref) == 0:
                ...
                self._log(f"Attributes for reference pyramid in level {level}: {attributes_ref}")
            ...
            if len(attributes_can) == 0:
                ...
                self._log(f"Attributes for candidate pyramid in level {level}: {attributes_can}")
    ...
    def _add_repeated_matches(
        ...
        list_group_by = [
            self.prefix_reference_data + level + matching_col_suffix for level in upper_levels
        ] + [col_reference_name]
    ...
    def _reorder_match_columns(
        ...
        cols_order_simple = []
        for level in levels_to_match:
            cols_order_full.append(self.prefix_candidate_data + level + matching_col_suffix)
            cols_order_simple.append(self.prefix_candidate_data + level + matching_col_suffix)
            ...
            cols_order_full.append(self.prefix_reference_data + level + matching_col_suffix)
            cols_order_simple.append(self.prefix_reference_data + level + matching_col_suffix)
